ppApp.directive('form', ['$parse', 'SettingService', '$rootScope', function($parse, SettingService, $rootScope) {
    return {
        restrict: 'EA',
                ...
                pre: function($scope, $elem, $attrs) {
                    ...
                    $scope.validateField = function(data, single) {
                        ...
                        if (data.required && data.value && (data.value === undefined || data.value === "" || data.value.length === 0 || data.value === $rootScope.sketch)) {
                            ...
                            data.errors.push({
                                type: 'required',
                                message: data.label + ' is too short. Must be min ' + data.minLength + ' chars long.'
                            });
                        } else if (data.maxLength !== undefined && data.value !== undefined && data.value.length > data.maxLength) {
                            data.errors.push({
                                type: 'required',
                                message: data.label + ' is too long. Must be max ' + data.minLength + ' chars long.'
                            });
                        } else if (data.minValue !== undefined && data.value !== undefined && data.value < data.minValue) {
                            data.errors.push({
                                type: 'required',
                                message: data.label + ' is too small. Must be min ' + data.minValue + '.'
                            });
                        } else if (data.maxValue !== undefined && data.value !== undefined && data.value > data.maxValue) {
                            data.errors.push({
                                type: 'required',
                                message: data.label + ' is too big. Must be max ' + data.maxValue + '.'
                            });
                            ...
                            data.errors.push({
                                type: 'invalid',
                                message: data.label + ' is invalid e-mail address.'
                            });
                        } else if (data.type == 'number' && $elem.hasClass('ng-invalid-number')) {
                            data.errors.push({
                                type: 'invalid',
                                message: data.label + ' must be a valid number.'
                            });
                        }
                    ...
                    $elem.bind('submit', function() {
                        ...
                        if (!$scope.validate()) {
                            SettingService.show_message_popup('Form has errors', 'Please correct the errors and submit again.');
                            return false;
                        }
